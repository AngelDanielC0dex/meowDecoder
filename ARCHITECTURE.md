...
│   ├── media/       #   optimize-image.ts (downscale+WebP en cliente, sin deps)
...
│   ├── components/  #   ui/ (Button, Modal, ConfirmDialog), audio/, results/,
│   │                #   cats/, cards/, medical/ (VaccineChecklist, CatCareTips),
│   │                #   ads/, layout/, decor/
...
├── content/         # Base de conocimiento tipada (vocalizaciones, FAQ,
│                    # vacunas, avisos de cuidado felino, artículos)
...
**Capa decorativa (`presentation/components/decor/`):** piezas puramente visuales y `aria-hidden`, sin lógica ni datos — `PawBackground` (fondo de huellas que "caminan": estelas L/R alternas en dirección aleatoria, cliente, solo opacity/transform, reduced-motion = estático), `Paw` (glifo SVG reutilizable), `CatLogo` (wordmark), `CatFace` (gato del hero con orejas rosas y bigotes) y `RopeCat` (gato colgando del navbar en la landing). Las animaciones provenientes de SCSS se **compilan a CSS Modules** estáticos (el proyecto no usa Sass): los `@for`/`@mixin`/funciones de color de Sass se expanden a keyframes y hex fijos, y cualquier reset global se *scopea* al componente. Todas respetan `prefers-reduced-motion` y animan solo `opacity/transform` (GPU).
...
- **UI reutilizable** — `ui/Button` (variantes primary/secondary/ghost/danger); `ui/Modal` (diálogo accesible con portal, focus-trap, Escape, scroll-lock, tokens de tema — sustituye `confirm()`/`alert()` nativos); `ui/ConfirmDialog` (confirmación con variante destructive). Usados en borrar gato, borrar dosis de vacuna y cualquier acción irreversible.
- **Optimización de imágenes** — `infrastructure/media/optimize-image.ts`: `File → Blob` WebP vía `createImageBitmap` + canvas, downscale al lado mayor, orientación EXIF respetada. Presets: `avatar` (≤512 px, q=0.85) y `card` (≤1080 px). Límite de entrada 10 MB. Usado en `CatManager` (avatar) y `CardDesigner` (foto de tarjeta).
...
- **Historial médico / vacunas** — `content/vaccines.ts` (catálogo + reglas por región: legal/recomendada/opcional + WSAVA core), `content/cat-care-tips.ts` (6 avisos de cuidado felino bilingües: chocolate, ibuprofeno, cebolla/ajo, lirios, leche, anticongelante), store IndexedDB v3 `vaccinations`, `VaccinationRepository`, `VaccineChecklist` (**registrado**, selector región, historial completo de dosis con chips borrables vía `ConfirmDialog`) en `/[locale]/medical`. `CatCareTips` (server component, rejilla danger/warn). **Carnet descargable/imprimible** (window.print + `@media print`) + export JSON.
...
- **Tarjetas de presentación** — `/[locale]/cards` (registrado). `Cat` extendido (`birthDate`, `bio`, `cardTemplate`, `showHoroscope`); foto en almacén IndexedDB v4 `catPhotos` (blob optimizado vía `infrastructure/media/optimize-image.ts`, borrado en cascada con el gato); `normalizeCat` da defaults a gatos antiguos. `domain/cat/zodiac.ts` (signo desde fecha, puro+testeado) + `catAgeYears`. UI `CardDesigner` + renderizador **canvas** `card-render.ts` (3 diseños: **Minimal**=claro+filete rosa, **Sticker**=fondo huellas+badges, **Premium**=oscuro+doble filete rosa/dorado+serif; una sola fuente para preview y descarga PNG, sin dependencias ni CORS). `wrapText` respeta saltos de línea (`\n`). Horóscopo opcional (checkbox) con frase por signo en i18n (`zodiac`). i18n agnóstico en el renderer (recibe textos ya localizados). **Compartir + QR** (`card-share.ts` codifica los datos en base64url; `ShareCard` usa Web Share API con el PNG real donde se pueda + iconos sociales + QR (`qrcode`, local) que apuntan a la página pública **`/[locale]/card?d=…`** (`PublicCardView`, reconstruye la tarjeta sin foto, noindex pero crawleable para previews). La foto NO viaja en la URL/QR (es blob local).
